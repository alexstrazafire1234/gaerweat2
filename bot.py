...
async def get_stats():
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        # Список возможных эндпоинтов для статистики
        endpoints = [
            "/stats/minimal",
            "/stats",
            "/api/stats",
        ...
        for endpoint in endpoints:
            try:
                ...
                logger.info(f"Trying {endpoint}: {resp.status_code} - {resp.text[:200]}")
                if resp.status_code == 200:
                    data = resp.json()
                    logger.info(f"Raw stats data from {endpoint}: {data}")
                ...
                logger.error(f"Error fetching stats from {endpoint}: {e}")
                continue
        
        logger.warning("All stats endpoints failed")
...
async def get_active_ips(limit=15):
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        # Список возможных эндпоинтов для получения списка пользователей/IP
        endpoints = [
            "/stats/users",
            "/stats/active",
            "/stats/clients",
        ...
        for endpoint in endpoints:
            try:
                resp = await client.get(f"{API_URL}{endpoint}")
                logger.info(f"Trying IPs endpoint {endpoint}: {resp.status_code} - {resp.text[:200]}")
                if resp.status_code == 200:
                    data = resp.json()
                    logger.info(f"Raw IPs data from {endpoint}: {data}")
                    ...
                    if isinstance(data, list):
                        ...
                        for key in ['ips', 'addresses', 'users', 'data', 'active_ips', 'clients', 'peers', 'connections']:
                            if key in data:
                                val = data[key]
                ...
                logger.error(f"Error fetching IPs from {endpoint}: {e}")
                continue
        
        logger.warning("All IP endpoints failed")
    return []
